...
Use auto repair to automatically heal instances which are marked unhealthy due to some <a href="/user/references/threshold-definitions.html">Threshold</a> violation or missing <a href="/user/references/heartbeat-monitors.html">Heartbeat</a>. Notifications are sent when an auto repair action is executed. Event component defined in a platform has an associated repair action specific to the component. The recipe for healing a component differs from one another. There are different set of instructions executed for compute repair then for tomcat repair
